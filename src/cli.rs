...
impl<T: CliConfig> Cli<T> {
    ...
    pub fn get_command(cmd: CliCommand) -> ::clap::Command {
        match cmd {
            CliCommand::Two => Self::cli_two(),
            CliCommand::Uno => Self::cli_uno(),
        }
    }
    pub fn cli_two() -> ::clap::Command {
        ::clap::Command::new("")
            .arg(
                ::clap::Arg::new("frontdoor")
                    .long("frontdoor")
    ...
    pub async fn execute(
        ...
        match cmd {
            CliCommand::Two => self.execute_two(matches).await,
            CliCommand::Uno => self.execute_uno(matches).await,
        }
    }
    pub async fn execute_two(&self, matches: &::clap::ArgMatches) -> anyhow::Result<()> {
        let mut request = self.client.two();
        if let Some(value) = matches.get_one::<::std::string::String>("frontdoor") {
            request = request.body_map(|body| body.frontdoor(value.clone()));
        }
        if let Some(value) = matches.get_one::<::std::string::String>("gateway") {
            request = request.body_map(|body| body.gateway(value.clone()));
        }
        self.config.execute_two(matches, &mut request)?;
...
impl CliCommand {
    pub fn iter() -> impl Iterator<Item = CliCommand> {
        vec![CliCommand::Two, CliCommand::Uno].into_iter()
    }
}
